...
<body>
	<div id="header">
		...
		<div>
   			<a href="${pageContext.request.contextPath }/manager/homeCostServlet?action=list1">返回历史记录</a>
		...
		<form action="${pageContext.request.contextPath }/manager/homeCostServlet" method="get">
			...
			<table>
				...
				<tr>
					...
					<td><input id="hostname" name="name" type="text" placeholder="请输入名称"value="${requestScope.homeCost.name}"/></td>
					<td><input id="money" name="money" type="text"placeholder="请输入金额" value="${requestScope.homeCost.money}"/></td>
					<td><input id="mess" name="mess" type="text" placeholder="请输入记账备注" value="${requestScope.homeCost.mess}"/></td>
					<td><input name="date" type="text"placeholder="日期系统自动录入"value="${requestScope.homeCost.date}"disabled="disabled" /></td>
					<td><input id="table submit"type="submit" value="提交"/></td>
				</tr>
			</table>
		</form>
	</div>
